...
public class BookShelf {
	...
	public boolean queryBookFromName(String name) {
		for(int i=0;i<books.size();i++) {
			if (books.get(i).getName()==name) {
				...
				System.out.printf("ISBN：%s;书名：%s;作者：%s;借阅情况：%s\n",book.getISBN(),book.getName(),book.getAuthor(),book.getBorrwedString());
				return true;
			}
		}
	...
	public void showAllBooks() {
		for(int i=0;i<books.size();i++) {
			System.out.printf("书名：%s;作者：%s;ISBN：%s;借阅情况：%s;\n",books.get(i).getName(),books.get(i).getAuthor(),books.get(i).getISBN(),books.get(i).getBorrwedString());
		}
	}
